...
void SkinnedDiffuseMaterial::UpdateEffectVariables(const GameContext& gameContext, ModelComponent* pModelComponent)
{
	...
	if (m_pBoneTransformsVariable)
	{
		//m_pBoneTransformsVariable->set
		m_pBoneTransformsVariable->SetMatrixArray(&pModelComponent->GetAnimator()->GetBoneTransforms().at(0)._11, 0 , pModelComponent->GetAnimator()->GetBoneTransforms().size());
	}
}
