...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float2 texCoord, float4 col, float2x2 uvRotation)
{
    ...
    output.Position = mul(float4(pos, 1), (float4x4) gWorldViewProj);
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
    ...
    float sizeDiv2 = (size * 0.5f);

	//Vertices (Keep in mind that 'origin' contains the center of the quad
    topLeft = float3((-sizeDiv2), ( sizeDiv2), 0.0f);
    topRight = float3((sizeDiv2), (sizeDiv2), 0.0f);
    bottomLeft = float3((-sizeDiv2), (-sizeDiv2), 0.0f);
    bottomRight = float3((sizeDiv2), (-sizeDiv2), 0.0f);
