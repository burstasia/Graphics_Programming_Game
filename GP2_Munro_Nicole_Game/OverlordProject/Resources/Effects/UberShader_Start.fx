...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	specularColor = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord) * specularStrength;
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	float3 result = mul((mul(fresnel, fresnelMask)), environmentColor);
	return result;
}
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	float3 cubeMapSample = gCubeEnvironment.Sample(gTextureSampler, reflectedVector) * gRefractionStrength;
	
	
	return environmentColor * cubeMapSample;
}
