...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	float3 basePoint, top, left, right, spikeNormal, spikeDirection;

	//Step 1. Calculate The basePoint
    top = vertices[0].Position;
    left = vertices[1].Position;
    right = vertices[2].Position;
    basePoint = top + left + right;
    basePoint /= 3.0f;
	//Step 2. Calculate The normal of the basePoint
    top = vertices[0].Normal;
    left = vertices[1].Normal;
    right = vertices[2].Normal;
    spikeNormal = top + left + right;
    spikeNormal /= 3.0f;
    ...
    spikeNormal = cross(top - right, top - left);
